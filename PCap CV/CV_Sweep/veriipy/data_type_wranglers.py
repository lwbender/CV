...
#    - date-time delimter: \t, ' ', ??
#    - time delimiters: ':' ??
#    - timezone is like "+hh:mm" appended to the end of the string.
...
def datetime_handler(date_in, tz_out=None, r_type='datetime'):
	...
	#
	# numpy.datetime64...
	# these types are a HUGE pain in my ... neck. for some reason, they keep chaning. they appear to be based on an array object, so once upon
...
def guess_type(x, int_to_float=False):
	...
	if x.count('/')==2 or x.count('-') in (2,3):
		...
		try:
			...
			return str(x)
		#
	elif x.count('.')==1 and str.isnumeric(x.replace('.','')):
		# probably a float:
		return float(x)
	elif str.isnumeric(x):
		if int_to_float:
			return float(x)
		else:
			return int(x)
